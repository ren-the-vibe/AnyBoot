...
export function generateGrubCfg(isos: IsoFile[]): string {
  const lines: string[] = [
    ...
    `set pager=1`,
    ``,
    `# Load modules.  On UEFI without Secure Boot insmod works normally.`,
    `# On UEFI with Secure Boot insmod may be blocked, but the signed`,
    `# binary already has fat, loopback, iso9660, search, etc. built in;`,
    `# the insmod calls simply fail silently and execution continues.`,
    `insmod all_video`,
    `insmod loopback`,
    `insmod iso9660`,
    `insmod fat`,
    ...
    `insmod part_msdos`,
    ``,
    `# Find the BOOTANY data partition`,
    `search --no-floppy --label BOOTANY --set=root`,
    ``,
    `# Fallback: if label search failed (e.g. NTFS not readable by the`,
    `# signed UEFI binary), derive data partition from the boot device.`,
    `# Our layout is always ESP=gpt1, BIOS-boot=gpt2, Data=gpt3.`,
    `if [ "$grub_platform" = "efi" ]; then`,
    `    if ! [ -d ($root)/iso ]; then`,
    `        regexp --set=1:bootdisk '(hd[0-9]+)' "$root"`,
    `        if [ -n "$bootdisk" ]; then`,
